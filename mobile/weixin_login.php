...
if($_GET['code']){
	...
	if($json['openid']){
		...
		else
		{
			...
			$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('users') . '(user_name , password, aite_id , sex , reg_time , is_validated,froms,headimg,parent_id) VALUES ' . "('$info[name]' , '$user_pass' , '$info_user_id' , '$info[sex]' , '" . gmtime() . "' , '0','mobile','$info[headimgurl]','$up_uid')";
			$GLOBALS['db']->query($sql);
			$tag = 1; //第一次注册标记
		}
