...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1')
        {
	        ...
	        $user_frozen_money = get_user_frozen_money($account['user_id']);//用户冻结金额
 ...
 * @return  int
 */
function get_user_frozen_money($user_id)
{
    $sql = "SELECT SUM(frozen_money) FROM " .$GLOBALS['ecs']->table('account_log').
           " WHERE user_id = '$user_id'";
