 ...
 * 
 */
 //palenggege修改   可以把这整个函数替换成我修改的 
function send_sms($supplierinfo='',$position=1,$kehu='',$order_sn=''){
	...
	//付款通知客户
        ...
        if($position==2&&$GLOBALS['_CFG']['dayu_order_pay'] == 1&&$kehu!="")
				   {
	...
	//下订单通知商家	
		...
		if ($position == 1){
			if ($_CFG['dayu_order_placed'] == '1' && $_CFG['dayu_shop_mobile'] != '')
		    {
		    	$phones = explode(',',$_CFG['dayu_shop_mobile']);
		    	array_filter($phones);
	...
	$moban=trim($GLOBALS['_CFG']['dayu_order_placed_tpl']);
	   ...
	   foreach($phones as $phone){
		...
		elseif ($position == 2){
			...
			if ($GLOBALS['_CFG']['dayu_order_payed'] == 1&&$_CFG['dayu_shop_mobile']!="")
		    {   
		    	$phones = explode(',',$_CFG['dayu_shop_mobile']);
		    	array_filter($phones);
...
//通知入驻商家短信，入驻商后台短信设置要填写手机号码和发短信才管用
