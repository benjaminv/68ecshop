...
/**
 * QQ120029121 后台对上传文件的处理类(实现图片上传，图片缩小， 增加水印)
 * 需要定义以下常量
 ...
 *  define('PC_ROOT_PATH',                     '网站根目录')
 *
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: cls_image.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
class cls_image
{
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        if (empty($dir))
        {
            /* 创建当月目录 */
            $dir = date('Ym');
            $dir = PC_ROOT_PATH . $this->images_dir . '/' . $dir . '/';
        }
        else
        {
            /* 创建目录 */
            $dir = PC_ROOT_PATH . $this->data_dir . '/' . $dir . '/';
            if ($img_name)
            {
        ...
        {
            return str_replace(PC_ROOT_PATH, '', $img_name);
        }
        else
        {
    ...
    {
		 $img=PC_ROOT_PATH.$img;//prince qq1 2 029121 统一存在
        ...
        {
            return str_replace(PC_ROOT_PATH, '', str_replace('\\', '/', realpath($img)));
        }
        ...
        if (empty($path))
        {
            $dir = PC_ROOT_PATH . $this->images_dir . '/' . date('Ym').'/';
        }
        else
        {
            $dir = PC_ROOT_PATH .$path;//prince 统一存放
        }
        ...
        /* 生成文件 */
	/* wei2 修改 start by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
 	...
 	/* wei2 修改 end by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
        else
        {
        ...
        {
            return str_replace(PC_ROOT_PATH, '', $dir) . $filename;
        }
        else
        {
    ...
    function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65)
    {
        ...
        {
            return str_replace(PC_ROOT_PATH, '', str_replace('\\', '/', realpath($filename)));
        }
        ...
        if ($path)
        {
            return str_replace(PC_ROOT_PATH, '', str_replace('\\', '/', $path));
        }
        else
        {
            $this->error_msg = $GLOBALS['_LANG']['writting_failure'];
