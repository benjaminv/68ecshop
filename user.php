...
'act_forget_pass', 're_pass', 'open_surplus_password', 'close_surplus_password','query_pay_status'
);
...
/*余额额支付密码_更改_START_www.68ecshop.com*/
'act_forget_pass', 're_pass', 'auction_list', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code','tuijian'
); // 代码修改
   // By
   // www.68ecshop.com
/* 余额额支付密码_更改_END_www.68ecshop.com */
...
function action_oath ()
{
	...
	if($type == "weixin")
	{
		header("location:weixin/login.php");
		exit();
	}
...
//		$smarty->assign('payment_list', $payment_list);
	...
	$order['int_pay_status'] = $order['pay_status'];
	/* 订单 支付 配送 状态语言项 */
	$order['order_status'] = $_LANG['os'][$order['order_status']];
	$order['pay_status'] = $_LANG['ps'][$order['pay_status']];
	$order['shipping_status_id'] = $order['shipping_status']; // 代码增加 By
...
//PRINCE QQ 120029121
function action_query_pay_status(){
...
function action_affiliate ()
{
	...
	$dengji=intval(isset($_REQUEST['dengji']) ? $_REQUEST['dengji'] : 0);
...

{
    ...
    $huiyuan=get_distrib_user_info($user_id,$dengji);


   	$smarty->assign('huiyuan', $huiyuan);

    $result['message'] = $smarty->fetch('library/fencheng.lbi');
...
function get_distrib_user_info($user_id,$level)
{
	...
	if($count)
	{
		 $sql = "SELECT user_id, user_name, email, mobile_phone, last_login, rank_points, pay_points, reg_time ".
		 ...
		 foreach($list as $key => $val)
		 {
			  ...
			  $arr[$key]['email'] = $val['email'];
			   $arr[$key]['mobile_phone'] = $val['mobile_phone'];
			   $arr[$key]['reg_time'] =  local_date('Y-m-d H:i:s', $val['reg_time']);

			  $arr[$key]['last_login'] =  local_date('Y-m-d H:i:s', $val['last_login']);
