...
require_once( '../' . ADMIN_PATH . '/includes/lib_goods.php');
//include_once(PC_ROOT_PATH . '/includes/cls_image.php');
...
{
	echo '<center><br>请先添加商品成功后再编辑相册！</center>';
	exit;
}
//echo $goods_attr_id;
...
{
	$img_id_qq_wx_120029121 = empty($_REQUEST['img_id']) ? 0 : intval($_REQUEST['img_id']);
	$sql_qq_wx_120029121 = "update ". $GLOBALS['ecs']->table('goods_gallery') ." set is_attr_image='0' where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' ";
	$db->query($sql_qq_wx_120029121);
	$sql_qq_wx_120029121 = "update ". $GLOBALS['ecs']->table('goods_gallery') ." set is_attr_image='1' where img_id='$img_id_qq_wx_120029121' ";
	$db->query($sql_qq_wx_120029121);
	clear_cache_files();
}
...
*{ margin:0; padding:0;}
body{font-size:12px;}
.gallery_box{width:100%;height:auto; margin:10px 0px 0px; }
.gallery_img_box{float:left;width:36%;border:1px solid #eeeeee;padding:10px;margin-left:5%;margin-bottom:11px;text-align:center;position:relative;}
.gallery_img{width:100px;height:100px;}
.blank{height:10px; line-height:10px; clear:both; visibility:hidden;}
.shuxingtupian{color:#fff;background:#ff3300;}

#box{ margin:0px 5%; width:90%; min-height:35px; background:#FF9; margin-top:10px;}
...
function handle_gallery_image_attr($goods_id, $goods_attr_id, $image_files)
{
    ...
    $proc_thumb = (isset($GLOBALS['shop_id']) && $GLOBALS['shop_id'] > 0)? false : true;
        ...
        if ($flag)
        {
			...
			//生成商品详情页图片
           $img_url = $GLOBALS['image']->make_thumb($img_original , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
            ...
            /* 重新格式化图片名称 */
            $img_original = ms_reformat_image_name('gallery', $goods_id, $img_original, 'source');
            $img_url = ms_reformat_image_name('gallery', $goods_id, $img_url, 'goods');
            $thumb_url = ms_reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb');
