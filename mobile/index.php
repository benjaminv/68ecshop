...
//added by prince qq 12029121 20160628 start
/*if(!isset($_SESSION['location_suppId']) || $_REQUEST['to_supplier']){
	$user_id=$_SESSION['user_id']?$_SESSION['user_id']:0;
	$weixin_user = $GLOBALS['db']->getRow( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_user') . " WHERE `ecuid` = '$user_id'" );
	...
	if($latitude || $longitude ){
		...
		require_once(dirname(__FILE__) . '/includes/Princehash.php');
		$geohash = new Princehash();
		...
		$suppId=$supplier_list[0]['supplier_id'];
		$_SESSION['location_suppId']=$suppId;
		header("Location:supplier.php?suppId=$suppId "); 
	}
}*/
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $smarty->assign('extpintuan_goods',   index_get_extpintuan());      // 拼团商品 QQ 497401495

	 $smarty->assign('cut_goods',   index_get_cut());      // 拼团商品 QQ 497401495
...
<meta property="qc:admins" content="3454222175763774076763757572145" /> 