...
require_once(ROOT_PATH . 'includes/lib_goods1.php');
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
	...
	elseif ($_REQUEST['act'] == 'list' && $_REQUEST['supplier_status']=='2')
	{
		$ur_here = $_LANG['01_goods_list_pass4'];
	}
	...
	if($_REQUEST['act'] == 'trash'){
        ...
        if(isset($_REQUEST['supplier_status']) && $_REQUEST['supplier_status'] == 0){
			$ur_here='未审核商品';
		}elseif(isset($_REQUEST['supplier_status']) &&  $_REQUEST['supplier_status'] == 1){
			$ur_here='审核通过商品';
		}elseif(isset($_REQUEST['supplier_status']) &&  $_REQUEST['supplier_status'] == -1){
			$ur_here='审核未通过商品';
		}elseif(isset($_REQUEST['supplier_status']) &&  $_REQUEST['supplier_status'] == 2){
			$ur_here='平台转移商品';
		}else{
			$ur_here='全部商品';
		}
    ...
    _wap_assign_header_info($ur_here,$ur_here,0,1);
    _wap_assign_footer_order_info();
    _wap_display_page($htm_file);
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	$smarty->assign('supplier_path', SUPPLIER_PATH);
	$smarty->assign('supplier_status', $_REQUEST['supplier_status']);
    ...
    if ($is_add)
    {
        ...
        if (!empty($_COOKIE['YPCP']['last_choose']))
        {
            $last_choose = explode('|', $_COOKIE['YPCP']['last_choose']);
        }
        $goods = array(
            'goods_id'      => 0,
            'goods_desc'    => '',
    ...
    /* 创建 html editor */
   create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 百度编辑器 */
    ...
    //$smarty->assign('brand_list', get_brand_list(true));//pr ince   20170512
    $smarty->assign('brand_list', get_brand_list());
    ...
    if($_REQUEST['act'] == 'add'){
   	 	_wap_assign_header_info('添加商品','添加商品',0,1);
		_wap_assign_footer_order_info();
		_wap_display_page('goods_add.htm');
	}else{
    	_wap_assign_header_info('编辑商品','编辑商品',0,1);
		_wap_assign_footer_order_info();
		_wap_display_page('goods_info.htm');
	}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$supplier_id = isset($_SESSION['supplier_id']) ? intval($_SESSION['supplier_id']) : $_COOKIE['YPCP']['supplier_id'];
    ...
    $goods_id = $is_insert ? $db->insert_id() : $_REQUEST['goods_id'];
	 ...
	 /*---------如果该商品分成金额大于0  自动成为分销商品 mod by prince qq 120029121 20160714-----------*/
     if ($cost_price > 0 and $is_distrib_goods == 1 )     {
				 //$is_distrib_goods_id = $db->getOne('SELECT MAX(goods_id) AS is_distrib_goods_id FROM ' .$ecs->table('goods'));
				$sql = "insert into ".$GLOBALS['ecs']->table('ecsmart_distrib_goods')." (`distrib_time`,`start_time`,`end_time`,`distrib_money`,`distrib_type`,`goods_id`) 
				value (0,0,0,'$cost_price',1,'$goods_id ')";
				  $db->query($sql);
	 }
     /*--------------------- mod by prince qq 120029121 2016071---------*/ 
...
/*编辑商品分成金额成为分销商品*/ 
     if ($cost_price > 0){
        if ($cost_price <= $shop_price )     

                 {
				 
				$sql = "SELECT goods_id FROM " .$GLOBALS['ecs']->table('ecsmart_distrib_goods'). " WHERE goods_id = $goods_id";
				
				$distrib_goods_id = $GLOBALS['db']->getOne($sql);

				if (empty($distrib_goods_id))//如果该商品还不是分销商品
			...
			$sql = "insert into ".$GLOBALS['ecs']->table('ecsmart_distrib_goods')." (`distrib_time`,`start_time`,`end_time`,`distrib_money`,`distrib_type`,`goods_id`) 
...

                {
                    ...
                    $sql = "UPDATE " .$ecs->table('ecsmart_distrib_goods'). " SET distrib_money = '$cost_price' WHERE goods_id = $goods_id";
					
					 $db->query($sql);//修改分销商品分成金额
    ...
    $original_img = ms_reformat_image_name('goods', $goods_id, $original_img, 'source');
    $goods_img = ms_reformat_image_name('goods', $goods_id, $goods_img, 'goods');
    $goods_thumb = ms_reformat_image_name('goods_thumb', $goods_id, $goods_thumb, 'thumb');
    if ($goods_img !== false)
    {
    ...
    if (isset($img))
    {
        ...
        if (empty($is_url_goods_img))
        {
            $img = ms_reformat_image_name('gallery', $goods_id, $img, 'source');
            $gallery_img = ms_reformat_image_name('gallery', $goods_id, $gallery_img, 'goods');
			$gallery_img = ms_reformat_image_name('gallery', $goods_id, $goods_img, 'goods');
        }
        else
        {
            $img = $url_goods_img;
        ...
        $gallery_thumb = ms_reformat_image_name('gallery_thumb', $goods_id, $gallery_thumb, 'thumb');
    ...
    /* 记录上一次选择的分类和品牌 */
    setcookie('YPCP[last_choose]', $catgory_id . '|' . $brand_id, gmtime() + 86400);
    /* 清空缓存 */
    clear_cache_files();
	...
	if($is_insert)
	 {
		$link[3] = array('href' => 'goods.php?act=list' , 'text' => '返回商品列表');
	 }
	 else
	{
		$link[3] = array('href' => 'goods.php?act=list&supplier_status=' . $_POST['supplier_status'], 'text' => '返回商品列表');
	 }
...
{  
    ...
    if (isset($_POST['type']))
    {
        ...
        elseif ($_POST['type'] == 'restore')
        {
            /* 检查权限 */
            admin_priv('remove_back');
              //商品总数    寒冰   2016.09.09     商家商品数量限制      qq   30 94 8 5552
        $goods_total = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND  supplier_id='.$_SESSION['supplier_id']);
		//店铺等级id
		$supplier_rank_id = $db->getOne('SELECT rank_id FROM '.$ecs->table('supplier').' WHERE  supplier_id='.$_SESSION['supplier_id']);
		//店铺等级配置
		$supplier_rank = $db->getRow('SELECT * FROM '.$ecs->table('supplier_rank').' WHERE  rank_id = '.$supplier_rank_id);
		if($goods_total >= $supplier_rank['goods_number']){
        $link[] = array('href' => 'goods.php?act=list', 'text' => '返回商品列表');
        sys_msg("您的店铺等级是【".$supplier_rank['rank_name']."】，目前只能添加".$supplier_rank['goods_number']."件商品！！请联系管理员提升店铺等级！！", 1, $link);
		 exit;
		}
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-begin
		/* 淘宝商品 */
        elseif ($_POST['type'] == 'to_taobao')
        {
            admin_priv('goods_manage');
            update_goods($goods_id, 'is_taobao', '1');
        }
		/* 取消淘宝商品 */
        elseif ($_POST['type'] == 'not_taobao')
        {
            admin_priv('goods_manage');
            update_goods($goods_id, 'is_taobao', '0');
        }
		/* 更新淘宝数据 */
        elseif ($_POST['type'] == 're_taobao')
        {
            admin_priv('goods_manage');
			
			$url = 'prince_shops.php?act=re_taobao&goods_id=' . $goods_id;
			
			//$aa = db_create_in($goods_id);
		...
		/* 更新阿里数据 */
        elseif ($_POST['type'] == 're_albb')
        {
            admin_priv('goods_manage');
			
			$url = 'albbprince_shops.php?act=re_taobao&goods_id=' . $goods_id;
		...
		/* 图片本地化 */
        elseif ($_POST['type'] == 'img_local')
        {
            admin_priv('goods_manage');
			
			$url = 'prince_shops.php?act=img_local&goods_id=' . $goods_id;
		...
		/* 采集评论 */
        elseif ($_POST['type'] == 'get_rate')
        {
            admin_priv('goods_manage');
			$ratenum = $_POST['ratenum'];

			
			$url = 'prince_shops.php?act=getAllRate&goods_id=' . $goods_id . '&ratenum=' . $ratenum;
       		yp_header("Location: $url\n");
			exit;
          }
		/* 采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1修改重量 */
        elseif ($_POST['type'] == 'pach_weight')
        {
            admin_priv('goods_manage');
			$goods_weight = floatval($_POST['goods_weight']);
            update_goods($goods_id, 'goods_weight', $goods_weight);
          }
		/* 翻译 */
        elseif ($_POST['type'] == 'translate')
        {
			...
			$url = 'prince_shops.php?act=translate&goods_id=' . $goods_id;
...
//-- 修改商品重量
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_weight')
{
    check_authz_json('goods_manage');
    $goods_id       = intval($_POST['id']);
    $goods_weight    = floatval($_POST['val']);
        if ($exc->edit("goods_weight = '$goods_weight',last_update=" .gmtime(), $goods_id))
        {
            clear_cache_files();
            make_json_result(number_format($goods_weight, 3, '.', ''));
        }
}
/*------------------------------------------------------ */
//-- 修改去淘宝状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_taobao')
{
    check_authz_json('goods_manage');
    $goods_id       = intval($_POST['id']);
    $is_taobao         = intval($_POST['val']);
    if ($exc->edit("is_taobao = '$is_taobao', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_taobao);
    }
}
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
...
//-- 修改货品价格
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_pro_price')
{
    check_authz_json('goods_manage');
    $product_id       = intval($_POST['id']);
    $pro_price       = floatval($_POST['val']);
    /* 货品价格 */
    $product = get_product_info($product_id, 'pro_price, goods_id');
    /* 修改货品价格 */
    $sql = "UPDATE " . $ecs->table('products') . " SET pro_price = '$pro_price' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
            clear_cache_files();
            make_json_result(number_format($pro_price, 2, '.', ''));
    }
}
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    $product['pro_price']      = $_POST['pro_price'];//采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1货品价格
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('products') . " (goods_id, goods_attr, product_sn, product_number, pro_price)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "','". $product['pro_price'][$key] . "')";//采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1保存货品价格
...
function ms_reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    if (!make_dir(PC_ROOT_PATH.$dir.'/'.$sub_dir))
    {
        return false;
    }
    if (!make_dir(PC_ROOT_PATH.$dir.'/'.$sub_dir.'/source_img'))
    {
        return false;
    }
    if (!make_dir(PC_ROOT_PATH.$dir.'/'.$sub_dir.'/goods_img'))
    {
        return false;
    }
    if (!make_dir(PC_ROOT_PATH.$dir.'/'.$sub_dir.'/thumb_img'))
    {
        return false;
    }
    switch($type)
    ...
    if ($position == 'source')
    {
        if (move_image_file(PC_ROOT_PATH.$source_img, PC_ROOT_PATH.$dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext))
        {
    ...
    {
        if (move_image_file(PC_ROOT_PATH.$source_img, PC_ROOT_PATH.$dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext))
        {
    ...
    {
        if (move_image_file(PC_ROOT_PATH.$source_img, PC_ROOT_PATH.$dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext))
        {
