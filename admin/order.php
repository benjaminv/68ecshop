...
elseif ($_REQUEST['act'] == 'list')
{
	...
	//新版拼团状态 Start  added by PRINCE
	$orders=$order_list['orders'];
    foreach ($orders as $key => $order_id){
		   if($orders[$key]['extension_code']=='extpintuan'){
			    $extpintuan = get_extpintuan_by_orderid($orders[$key]['order_id']);
                $orders[$key]['extpintuan_status'] =$extpintuan['status_desc'];
                $orders[$key]['pt_id'] =$extpintuan['pt_id'];
		   }
    }
	$order_list['orders']=$orders;
	//新版拼团状态 Start  added by PRINCE
	
	//云购状态 Start  added by PRINCE
	$orders=$order_list['orders'];
    foreach ($orders as $key => $order_id){
		   if($orders[$key]['extension_code']=='lucky_buy'){
			    $lucky_buy = get_lucky_buy_by_orderid($orders[$key]['order_id']);
                $orders[$key]['lucky_buy_act_id'] =$lucky_buy['act_id'];
                $orders[$key]['lucky_buy_id'] =$lucky_buy['lucky_buy_id'];
		   }
    }
	$order_list['orders']=$orders;
	//云购状态 Start  added by PRINCE
...
elseif ($_REQUEST['act'] == 'info')
{
	...
	if($order['extension_code']=='extpintuan'){
			    $extpintuan = get_extpintuan_by_orderid($order['order_id']);
                $order['extpintuan_status'] =$extpintuan['status_desc'];
                $order['pt_id'] =$extpintuan['pt_id'];

	}
    //新版拼团状态 Start
	
	//云购  Start  ADD BY P R I N C E  120029121
	if($order['extension_code']=='lucky_buy'){
			//取出云购码
			$sql = "select * from ". $GLOBALS['ecs']->table('lucky_buy_detail') . " where   order_id='".$order['order_id']."' ORDER BY code ASC ";
			$lucky_buy_codes =$GLOBALS['db']->getAll($sql);
       		$smarty->assign('lucky_buy_codes', $lucky_buy_codes);
			
			$sql = "select lucky_buy_id from ". $GLOBALS['ecs']->table('lucky_buy_detail') . " where   order_id='".$order['order_id']."' ORDER BY code ASC ";
			$lucky_buy_id =$GLOBALS['db']->getOne($sql);
			
			$sql = "select * from ". $GLOBALS['ecs']->table('lucky_buy') . " where   lucky_buy_id='".$lucky_buy_id ."' ";
			$lucky_buy_result =$GLOBALS['db']->getRow($sql);
       		$smarty->assign('lucky_buy_result', $lucky_buy_result);
	}
	//云购  END  ADD BY P R I N C E  120029121
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    @file_get_contents($autoUrl."mobile/weixin/weixin_remind.php?notice=4&is_one_user=".$order['user_id']."&order_id=".$order['order_id']);
  
      /* 如果需要，发短信  palenggege修改 */
        if ($GLOBALS['_CFG']['dayu_order_shipped'] == '1' && $order['mobile'] != '')
        {
					 ...
					 $moban=trim($GLOBALS['_CFG']['dayu_order_shipped_tpl']);  
...
/*------------------------------------------------------ */
    ...
    elseif (isset($_POST['unpay']))
    {
        ...
        $order          = order_info($order_id);
        if ($order['money_paid'] > 0  ||  $batch) // PRINCE QQ 120029121
        {
			  $show_refund = true;
        }
		if($batch){
    ...
    elseif (isset($_POST['remove']))
    {
        ...
        if (!$batch)
        {
            ...
            $db->query("DELETE FROM ".$ecs->table('extpintuan_orders'). " WHERE order_id = '$order_id'"); //拼团
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
	...
	$refund_type = $_REQUEST['refund'];         // ADD by PRINCE　QQ 120029121 
	$refund_note = $_REQUEST['refund_note'];    // ADD by PRINCE　QQ 120029121 
	...
	elseif ('unpay' == $operation)   // ADD by PRINCE
    {   
		require_once(ROOT_PATH . 'prince/lib/WxPay.Api.php');
        ...
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                $order_id = $order['order_id'];
				/*微信退款 Start  Add BY palenggege qq497401495  */
				if($refund_type==4){
				   $order_id=$order['order_id'];
				   ...
				   $order_sn=$order['log_id'].'-'.$order['money_paid'] * 100;
				   $order_sn=$order['out_trade_no']?$order['out_trade_no']:$order_sn;
				   $money_paid=$order['money_paid'];
				   $money_refund=$order['money_paid'];
				...
				/*微信退款  End   Add BY paleng QQ 497401495  */


                /* 标记订单为“未付款” */
				$arr = array(
					'order_status'    => $refund_type==4?OS_CANCELED:OS_CONFIRMED,
					'pay_status'    => $refund_type==4?PS_REFUNDED:PS_UNPAYED,
					'pay_time'      => 0,
					'money_paid'    => 0,
				...
				/* 记录log   mod by PRINCE QQ 120029121 */
				if($refund_type==4){
					order_action($order['order_sn'], OS_CANCELED, SS_UNSHIPPED, PS_REFUNDED, $action_note);
				}else{
					order_action($order['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, PS_UNPAYED, $action_note);
				}
        ...
        $sn_str = '以下订单当前状态不为已付款，无法设为未付款/已退款';
    }   // ADD by PRINCE
    elseif ('remove' == $operation)
    {
        foreach ($order_id_list as $id_order)
        {
            ...
            $db->query("DELETE FROM ".$ecs->table('extpintuan_orders'). " WHERE order_id = '$order[order_id]'"); //拼团
            $action_array = array('delivery', 'back');
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('unpay' == $operation)
    {
        ...
        /*微信退款 Start  Add BY PRINCE QQ 120029121  */
		if(@$_REQUEST['refund']==4){
		   $order_id=$order['order_id'];
		   $sql = "SELECT log_id FROM " . $GLOBALS['ecs']->table('pay_log') .
					" WHERE order_id = '$order_id' and order_type=0 ";
		   $order['log_id'] = $GLOBALS['db']->getOne($sql);
		   ...
		   if(!$wx_refund_status){
				/* 操作失败 */
				$links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
				sys_msg("操作微信退款失败，请检查该订单是否使用微信支付或者已在微信商户平台退款！", 0, $links);
		   }
		}
        /*微信退款  End   Add BY PRINCE QQ 120029121  */
		
        /* 标记订单为未付款，更新付款时间和已付款金额 */
        $arr = array(
			'order_status'  => @$_REQUEST['refund']==4?OS_CANCELED:OS_CONFIRMED,
            'pay_status'    => @$_REQUEST['refund']==4?PS_REFUNDED:PS_UNPAYED,
            'pay_time'      => 0,
            'money_paid'    => 0,
            'order_amount'  => $order['money_paid']
        );
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['ecs']->table('extpintuan_orders'). " WHERE order_id = '$order_id'");//拼团
    $action_array = array('delivery', 'back');
    del_delivery($order_id, $action_array);
    ...
    if ($result === false)
    {
        ...
        switch($filter['composite_status'])
        {
            ...
            case OS_SHIPPED_PART :
                ...
                break;
			// added by PRINCE 120029121	
            case 81 :
        			$where .= " AND o.extension_code = 'extpintuan' ";
                break;
            case 82 :
        			$where .= " AND o.extension_code = 'extpintuan' AND o.pt_status = 0 ";
                break;
            case 83 :
        			$where .= " AND o.extension_code = 'extpintuan' AND o.pt_status in (1,3,4) ";
                break;
            case 84 :
        			$where .= " AND o.extension_code = 'extpintuan' AND o.pt_status=3 ";
                break;
            case 85 :
        			$where .= " AND o.extension_code = 'extpintuan' AND (o.pt_status=1 or o.pt_lucky_order=1  ) AND o.shipping_status=0 ";
                break;
            case 86 :
        			$where .= " AND o.extension_code = 'extpintuan' AND o.pt_status = 2 ";
                break;
            case 87 :
        			$where .= " AND o.extension_code = 'extpintuan' AND (o.pt_status = 2 or (o.pt_status = 4 AND o.pt_lucky_order=0  )) AND o.pay_status=2 ";
                break;
            case 88 :
        			$where .= "AND o.pay_status=3 ";
                break;
			// added by P R I N C E QQ 1 2 0 0 2 9 1 2 1
            default:
                if ($filter['composite_status'] != -1)
                {
                    $where .= " AND o.order_status = '$filter[composite_status]' ";
        ...
        if($filter['supp']){
        	$sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.rebate_ispay, o.pay_name,o.wx_refund_status," .  //MOD by PRINCE
...
function get_extpintuan_by_orderid($order_id)

{
	...
	if($extpintuan['status']==1 || $extpintuan['status']==3 || $extpintuan['status']==4){
		$status="成功";
	}elseif($extpintuan['status']==2){
		$status="失败";
	}else{
		$status="进行中";
	}
			
	if($extpintuan['is_lucky_extpintuan']){
		if($extpintuan['status']==3){
			$status.='(待抽奖)';
		}elseif($extpintuan['status']==4){
			if($extpintuan['lucky_order']){
				$status.='(本订单已中奖)';
			}else{
				$status.='(本订单未中奖)';
			}
	...
	$extpintuan['status_desc']=$status;
	return $extpintuan;
}

//云购
function get_lucky_buy_by_orderid($order_id)
{
	$sql = "SELECT lb.* " .
				...
				" WHERE lbd.order_id=".$order_id." LIMIT 1 ";
