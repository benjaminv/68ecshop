...
//    $smarty->assign('cod_disabled',    $cod_disabled);
	...
	if ($flow_type == CART_LUCKY_BUY_GOODS||$flow_type == CART_EXTPINTUAN_GOODS)
    {
        $smarty->assign('cod_disabled', 1);
    }else{

       $smarty->assign('cod_disabled', 0);
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
        ...
        if($user_info['pay_points']> $keyong[0]){
        ...
        $kyjf=$keyong[0];
         ...
         $kyjf=$user_info['pay_points'];


		}

        $smarty->assign('kyjf', $kyjf);
        $smarty->assign('allow_use_integral', 1);
        $smarty->assign('order_max_integral', $keyong[0]);  
...
{	
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    if (isset($_SESSION['flow_type']) && intval($_SESSION['flow_type']) != CART_GENERAL_GOODS)
	    {
	        ...
	        $order['pt_level'] = $_SESSION['extpintuan_level']?$_SESSION['extpintuan_level']:0;//新版拼团 PRINCE
	    }
	    else
	    {
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    if ($order['extension_code']=='auction')
	    {
		 ...
		 /* 如果订单金额为0（使用余额或积分或红包支付），自动分成    提醒客户*/
	    if ($order['order_amount'] <= 0)
	    {
		  ...
		  /* 微信订单提醒 */  
					...
					@file_get_contents($wap_url."weixin/weixin_remind.php?notice=2&is_one_user=".$order['user_id']."&order_id=".$new_order_id); 
		
		   /*付款自动分成    寒冰  QQ  paleng*/
		     
			if ($GLOBALS['_CFG']['pay_fencheng'] == 1)
                {
				   include_once(ROOT_PATH . 'includes/lib_pay_fencheng.php');
				   get_pay_fencheng($new_order_id);
				}
		 
		    
	    }
		
		/* 修改用户砍价活动  PRINCE 120029121 */
		if ($order['extension_code']=='cut')
		{
			$sql = "UPDATE ". $ecs->table('cut') ." SET `order_times`=`order_times`+1 WHERE cut_id=".$_SESSION['cut_id'];
			$db->query($sql);
		}
		...
		{   
		   ...
		   if($_SESSION['extpintuan_pt_id']==0){
			    $end_time=$now_time+$_SESSION['extpintuan_time_limit']*60*60;
				$extpintuan = array(
					'user_id'    => $_SESSION['user_id'],
					'user_nickname'  => $_SESSION['extpintuan_nickname'],
					'user_head'  => $_SESSION['extpintuan_headimgurl'],
					'act_id'  => $_SESSION['extension_id'],
					'act_type'  => GAT_EXTPINTUAN,
					'is_lucky_extpintuan'  => $_SESSION['extpintuan_lucky_extpintuan'],
					'need_people' => $_SESSION['extpintuan_level'],
					'available_people' => $_SESSION['extpintuan_level']-1,
					'price' => $_SESSION['extpintuan_price'],
					'create_time'  => $now_time,
					'end_time'  => $end_time>$_SESSION['extpintuan_end_time']?$_SESSION['extpintuan_end_time']:$end_time
				);
				$db->autoExecute($ecs->table('extpintuan'), $extpintuan, 'INSERT');	
				$new_pt_id = $db->insert_id();
			}
			
			$extpintuan_orders = array(
					'pt_id'  => $_SESSION['extpintuan_pt_id']?$_SESSION['extpintuan_pt_id']:$new_pt_id,
					'act_id'  => $_SESSION['extension_id'],
					'act_user'  => $_SESSION['extpintuan_act_user'],
					'follow_user'    => $_SESSION['user_id'],
					'follow_user_nickname'  => $_SESSION['extpintuan_nickname'],
					'follow_user_head'  => $_SESSION['extpintuan_headimgurl'],
					'order_id' => $new_order_id ,
					'order_sn' => $order['order_sn'] ,
					'follow_time'  => $now_time
				);
			$db->autoExecute($ecs->table('extpintuan_orders'), $extpintuan_orders, 'INSERT');	
...
//                    }
              ...
              $aa=array(
               ...
               'supplier_name'=>$supplier_name,
	            ...
	            'vali_date'=>$sj
                            ...
                            $result = sendSMS($_REQUEST['mobile_phone'],$aa,$moban); 
	...
	require_once (ROOT_PATH . 'sms/sms.php');
	send_sms($supplier_ids,1);

   if($send){

    send_sms($supplier_ids,2,$order['mobile'],$order['order_sn']);
...
//palenggege修改结束
   ...
   @file_get_contents($autoUrl."weixin/weixin_remind.php?notice=1&is_one_user=".$order['user_id']."&order_id=".$order['order_id']);
  
    //@file_get_contents($autoUrl."weixin/weixin_remind.php?notice=6&is_one_user=".$order['user_id']."&order_id=".$order['order_id']);
