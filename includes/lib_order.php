...
function available_shipping_list($region_id_list,$suppid=0,$is_default_show=1,$shipping_code=null)
{
    ...
    */
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id  AND s.enabled = 1 AND s.supplier_id = '.$suppid.' ORDER BY s.shipping_order';
    ...
    return $GLOBALS['db']->getAll($sql);
     ...
     function zdfc($order_id){


       if($GLOBALS['_CFG']['distrib_style'] == 0)
		{
			include_once (ROOT_PATH . 'mobile/includes/lib_v_user.php');
    		...
    		$order_sn = $row['order_sn'];
			 ...
			 $sql = 'SELECT value FROM ' . $GLOBALS['ecs']->table('ecsmart_shop_config')." WHERE code = 'is_add_distrib'";
       
		 
		     $is_add_distrib = $GLOBALS['db']->getOne($sql);
			
			if($row['supplier_id'] == 0 || $is_add_distrib == 1)
			{
				...
				if($split_money > 0)
				{
					...
					for ($i=0; $i < $num; $i++)
					{
						...
						else
						{
							...
							insert_affiliate_log($order_id, $up_uid, $row['user_name'], $setmoney, $separate_by,"订单分成");
						}
					...
					$wap_url_sql = "SELECT `wap_url` FROM " . $GLOBALS['ecs']->table('weixin_config') . " WHERE `id`=1";
 ...
 */
 function get_pingtai_rebate_from_supplier($order_id)
 {
	...
	if($info['pay_status'] == PS_PAYED){
		...
		//收货确认的订单
		$split_money = get_split_money_by_fencheng($order_id);//订单产生的分成佣金
		...
		$add_time = gmtime();
		//佣金订单日志
		$sql = "INSERT INTO ".$ecs->table('supplier_rebate_log')." (order_id,order_sn,supplier_id,all_money,rebate_money,result_money,split_money,pay_id,pay_name,texts,add_time) VALUES ({$order_id},'{$order_sn}',{$supplier_id},'{$money}','{$rebate_money}','{$result_money}','{$split_money}',{$pay_id},'{$pay_name}','{$texts}',{$add_time})";
		$db->query($sql);
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    if (!in_array($refund_type, array(1, 2, 3,4)))  //MOD by PRINCE　QQ: 120029121
    {
        die('invalid params');
    }
...
//获取某一个订单的分成金额  寒冰
function get_split_money_by_fencheng($order_id)
{
