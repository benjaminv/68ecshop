...
function delete_goods($goods_id)
{
    ...
    $GLOBALS['db']->query($sql);
	 ...
	 $sql = "DELETE FROM " . $GLOBALS['ecs']->table('ecsmart_distrib_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
...
//        $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
        ...
        $filter['is_morning_market']= $_REQUEST['is_morning_market'];
...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
      ...
      if ($filter['is_morning_market'])
        {
            $where .= " AND is_morning_market = '$filter[is_morning_market]'";
        }
        /* 品牌 */
        if ($filter['brand_id'])
        {
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
        	$sql = "SELECT goods_id, goods_name, goods_type,is_morning_market, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
        	...
        	$sql = "SELECT goods_id, goods_name, goods_type, goods_sn,is_morning_market, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
