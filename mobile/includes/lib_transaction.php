            ...
            else
            {
                ...
                else
                {
					...
					@$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
                           ...
                           $zt = $GLOBALS['db']->getRow("SELECT status_back,back_id FROM " . $GLOBALS['ecs']->table('back_order') . " WHERE order_id = " . $row['order_id'] . " AND back_type= 4");

						   $status_back=$zt['status_back'];
					...
					//palengggege添加开始			
								...
								if (!empty($status_back))
				{
					 $row['tk'] ="<a href='user.php?act=back_order_detail&id=$zt[back_id]'>".$GLOBALS['_LANG']['bos'][$status_back]."</a>";
				}else{


								
							    $row['tk'] ="<a href='user.php?act=back_order&order_id=$row[order_id]&x=1'>退款</a>";
...
function get_order_goods($order)
{
	...
	{  
		...
		$goods_list[$goods_key]['back_id']=$back_order['back_id'];//palenggege添加
