...
class weixinapi{
	...
	//寒冰绑定用户
	function bindUser($wxid,$email,$pwd,$username='',$mobile_phone=''){
       ...
       $condi.="or mobile_phone='{$mobile_phone}'";
		...
		if($user){
		  ...
		  $autoreg = $GLOBALS['db']->getRow("SELECT * FROM `ecs_weixin_autoreg` WHERE `autoreg_id` = 1");//获取微信自动注册配置
		  $ec_name = $autoreg['autoreg_name'].$user_id;
		...
		//修改新注册的用户成为普通分销商 及更新openid  插入明文密码
		 $aite_id = 'weixin_'.$wxid;
		 
		$time=time();	
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET is_fenxiao=1,aite_id='$aite_id' , `passwd_weixin` =  '$pwd',`reg_time` =  '$time',`is_validated` =  0,`email`='$email' ,`user_name`='$ec_name',froms='mobile' WHERE user_id = '$user_id'");
		$id = $GLOBALS['db']->getOne("select uid from ".$GLOBALS['ecs']->table('weixin_user')." where fake_id='$wxid'");
		if($id > 0){
			$sql = "update ".$GLOBALS['ecs']->table('weixin_user')." set ecuid='$user_id',isfollow=1 ,ec_name='$ec_name' where fake_id='$wxid'";
		}else{
			$createtime = time();
	...
	function unBindUser($wxid){
		...
		$wx_id = "weixin_".$wxid;
		$sql = "update ".$GLOBALS['ecs']->table('users')." set aite_id='' where aite_id='$wx_id'";
		$GLOBALS['db']->query($sql);
	...
	**/
	...
	function queryKuaidi($wxid='oPsituCpCTsGEI-df2Km8qUB2kuA'){
		...
		if($info['ecuid'] > 0){
			...
			$order = $GLOBALS['db']->getAll("SELECT order_id,order_sn,invoice_no,shipping_name FROM " . $GLOBALS['ecs']->table('delivery_order') . " where user_id='{$info['ecuid']}' and add_time>'{$add_time}'");
			if(!$order){
			...
			foreach ($order as $k=>$o){
				...
				$kdurl =$GLOBALS['weburl']. "mobile/kuaidi_list.php?order_id=".$o['order_id'];
				$url="http://www.kuaidi100.com/query?type=".getKDname($o['shipping_name'])."&postid={$o['invoice_no']}";

				$result = $this->getcontent($url);
				
				$kuaidi = json_decode($result,true);
				if($kuaidi['message'] == 'ok'){
					$order[$k]['kuaidi']['context'] ="<a href='$kdurl'>{$kuaidi['data'][0]['context']}</a>";
				}else{
