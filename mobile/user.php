...
else
{
	if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger'))
	{
		...
		{ 
			//$url = $GLOBALS['ecs']->url()."user.php";  // prince qq 1 2 00 29121
...
if(in_array($action, $ui_arr))
{
	...
	else
	{
	 ...
	 $supplier_id_sql = "SELECT `supplier_id` FROM  " . $GLOBALS['ecs']->table('supplier') . " WHERE `user_id`='$user_id' and status=1";
	 $supplier_id =  $GLOBALS['db'] -> getOne($supplier_id_sql);
	if(! empty($supplier_id))
	{
		$is_supplier = 1;
		$smarty->assign('is_supplier',$is_supplier);
	}
	
	if (file_exists("extpintuan.php")) {
	$is_pintuan = 1;
	$smarty->assign('is_pintuan',$is_pintuan);
}

if (file_exists("cut.php")) {
   $is_cut = 1;
	$smarty->assign('is_cut',$is_cut);
}
if (file_exists("lucky_buy.php")) {
   $lucky_buy = 1;
	$smarty->assign('lucky_buy',$lucky_buy);
}
if (file_exists("pre_sale.php")) {
   $pre_sale = 1;
...
// 	);
	...
	$profile = array(
		...
		'user_id' => $user_id, 'user_name'=> $user_name,'email'=> $email,'sex' => isset($_POST['sex']) ? intval($_POST['sex']) : 0,'birthday' => $birthday,'other' => isset($other) ? $other : array()
...

	{
	   ...
	   if(!empty($_FILES['headimg']['name']))
		...
		$string = strrev($_FILES['headimg']['name']);
		$array = explode('.',$string);
		$type=$headimg["type"];//上传文件的类型 
		$size=$headimg["size"];//上传文件的大小 
		$tmp_name=$headimg["tmp_name"];//上传文件的临时存放路径 
		
		$adname = 'headimg/'.time().rand(10,100).'h.'.strrev($array[0]);
		//判断是否为图片 
		switch ($type){ 
		...
		if($_FILES['headimg']['size'] > 2048000)
		{
		
		 
		 show_message("头像文件过大", $_LANG['profile_lnk'], 'user.php?act=profile', 'info');
		 
		}else{
		
		    if($okType){ 
			   $error=$headimg["error"];//上传后系统返回的值 
			//把上传的临时文件移动到up目录下面 


			   move_uploaded_file($tmp_name,$adname); 
			
	           $headimgurl = 'mobile/'.$adname;


			   $headimgurl2 = $ecs->url.$adname;
	           $sql = 'UPDATE ' . $ecs->table('users') . " SET `headimg`='".$headimgurl."'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";

	           $db->query($sql);//ec原生头像
		
		       $sql = 'UPDATE ' . $ecs->table('weixin_user') . " SET `headimgurl`='".$headimgurl2."'  WHERE `ecuid`='" . $_SESSION['user_id'] . "'";
		...
		$sql="select count(user_id) from".$GLOBALS['ecs']->table('supplier_admin_user')."where uid='$_SESSION[user_id]'";

   if($GLOBALS['db']->getOne($sql)>0){

	$sql = 'UPDATE ' . $ecs->table('supplier_admin_user') . " SET `user_name`='$user_name'WHERE `uid`='" . $_SESSION['user_id'] . "'";
     ...
     $db->query($sql);
		...
		show_message($_LANG['edit_profile_success'], $_LANG['profile_lnk'], 'user.php?', 'info');
...

{
   $user = $GLOBALS['user'];
	...
	$rec_id = intval($_GET['order_id']);
	

   $sql = "select back_id from " . $ecs->table('back_order') . " where order_id='$rec_id' ";
   $back_id = $db->getOne($sql);

	if($back_id)
	{
		show_message('您已经申请过退款！', '返回', 'user.php?act=back_order_detail&id=' . $back_id);
		exit();
	}
    ...
    $order=$db->getRow("select * FROM " . $ecs->table('order_info') ."where order_id='$rec_id'");
   ...
   if($goods_id){
	...
	$goods = $db->getAll("SELECT o.*,g.* FROM " . $ecs->table('order_goods') . " as o left join ".$ecs->table('goods')." as g on o.goods_id = g.goods_id WHERE order_id = '$rec_id' and g.goods_id='$goods_id'");

	$order_goods=$db->getRow("select goods_number,goods_price from". $ecs->table('order_goods') ."where order_id='$rec_id' and goods_id='$goods_id'");



   }else{

       $goods = $db->getAll("SELECT o.order_id,g.* FROM " . $ecs->table('order_goods') . " as o left join ".$ecs->table('goods')." as g on o.goods_id = g.goods_id WHERE order_id = '$rec_id'");
...
function action_back_order_act ()
{
	...
	//$imgs = ($_POST['img_srcs']) ? implode(',', $_POST['img_srcs']) : '';
   ...
   for($i=0;$i<4;$i++){
        ...
        $imgs = ($path_img[0]) ? implode(',', $path_img) : '';
...
//	}
    ...
    $sql="select supplier_id from".$ecs->table('goods')."where goods_id='$id_value'";
...
function action_act_account()
{
	...
	$deposit_least_money = !empty($_CFG['deposit_least_money']) ?  $_CFG['deposit_least_money'] : 1;
	...
	if($surplus['process_type'] == 1)
	{
		...
		if($amount < $deposit_least_money)
	{
		show_message('单次提现金额不能少于'.$deposit_least_money.'元');
	}
		...
		if($surplus['rec_id'] > 0)
		{
		    $frozen_money = str_replace("-", "", $amount);
		    $info = date('Y-m-d H:i:s')."余额提现，资金冻结".$frozen_money."元";
		    log_account_change($_SESSION['user_id'], $amount, $frozen_money, 0, 0, $info);
...
function get_user_backorders ($user_id, $num = 10, $start = 0)
{
	...
	$sql = "SELECT bo.*, g.goods_name " . " FROM " . $GLOBALS['ecs']->table('back_order') . " AS bo left join " . $GLOBALS['ecs']->table('goods') . " AS g " . " on bo.goods_id=g.goods_id  " . " WHERE bo.user_id = '$user_id' ORDER BY add_time DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	...
	while($row = $GLOBALS['db']->fetchRow($res))
	{
        ...
        $row['status_back'] = $GLOBALS['_LANG']['bos'][$row['status_back']] . ($row['status_back'] == '3' && $row['back_type'] && $row['back_type'] != '4' ? ' (换回商品已寄出，请注意查收) ' : '');
		...
		foreach($row['goods_list'] as $k=>$v){
			 ...
			 $sql="select goods_thumb from".$GLOBALS['ecs']->table('goods') ."where goods_id= $v[goods_id]";
			
			 $row['goods_list'][$k]['goods_thumb']=$GLOBALS['db']->getOne($sql);
