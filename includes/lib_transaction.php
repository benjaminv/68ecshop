...
function edit_profile($profile)
{
	 ...
	 if (isset($profile['user_name']))
    {
        $cfg['user_name'] = empty($profile['user_name']) ? $cfg['username'] : $profile['user_name'];
		 ...
		 $rzs['user_name'] =empty($profile['user_name']) ? $cfg['username'] : $profile['user_name'];
    }
...
$sql="select count(user_id) from".$GLOBALS['ecs']->table('supplier_admin_user')."where uid='$profile[user_id]'";

   if($GLOBALS['db']->getOne($sql)>0){

   $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('supplier_admin_user'), $rzs, 'UPDATE', "uid = '$profile[user_id]'");
