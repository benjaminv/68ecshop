...
if ($event['event'] == "subscribe") { //用户关注
	...
	if(intval($followInfo['ecuid']) == 0 && $weixinconfig['reg_type'] == 1){
		...
		if(!empty($data['Ticket']) || $scene_id )
		{
			...
			$sql = "select parent_id from ".$GLOBALS['ecs']->table('weixin_login')." where token = '".$data['Ticket']."'";
			$up_id= $GLOBALS['db']->getOne($sql);
...
function kefutips($access_token, $openid){
    $url = 'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$access_token;
     $data = '{
						...
						"text":
						{
							 "content":"您好，正在为您转接客服，如客服忙碌，请添加微信：boot899911aa "
						}
			}';
    $result = https_request($url, $data);
}
