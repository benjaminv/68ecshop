...
function process_image($page = 1, $page_size = 100, $type = 0, $thumb= true, $watermark = true, $change = false, $silent = true)
{
    if ($type == 0)
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
          ...
          if (preg_match('/(http:\/\/)|(https:\/\/)/i', $row['original_img'])) {
  
             $row['original_img']=htmlspecialchars_decode($row['original_img']);
		
			  
			  copy(trim($row['original_img']), ROOT_PATH . 'temp/' .basename($row['original_img']));

			  
              $row['original_img']= 'temp/' . basename($row['original_img']);


			  $row['goods_img']='temp/' . basename($row['original_img']);
            ...
            if ($thumb)
            {
			 ...
			 $goods_img = $GLOBALS['image']->make_thumb(ROOT_PATH.$row['original_img'], $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height'], $dir);
    ...
    else
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
          ...
          if (preg_match('/(http:\/\/)|(https:\/\/)/i', $row['img_original'])) {
  
             $row['img_original']=htmlspecialchars_decode($row['img_original']);
		
			  
			  copy(trim($row['img_original']), ROOT_PATH . 'temp/' .basename($row['img_original']));

			  
              $row['img_original']= 'temp/' . basename($row['img_original']);


			  $row['img_url']='temp/' . basename($row['img_original']);
            ...
            if ($thumb)
            {
			 ...
			 $img_url = $GLOBALS['image']->make_thumb(ROOT_PATH.$row['img_original'], $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height'], $dir);
                ...
                $thumb_url = reformat_image_name('gallery_thumb', $row['goods_id'], $thumb_url, 'thumb');
				 ...
				 if($bb==1){
                    ...
                    $img_url = reformat_image_name('goods', $goods_id, $img_url, 'goods');
                    $sql = "UPDATE " .$GLOBALS['ecs']->table('goods_gallery'). " SET img_url='$img_url' WHERE img_id='$row[img_id]'";
                         $GLOBALS['db']->query($sql);
