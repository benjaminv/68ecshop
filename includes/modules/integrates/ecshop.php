...
class ecshop extends integrate
{
	...
	function edit_user ($cfg)
	{
		...
		if((! empty($cfg['md5password'])) && $this->field_pass != 'NULL')
		{
		    ...
		    $values1[] = $this->field_pass . "='" . $this->compile_password(array(
				'md5password' => $cfg['md5password'])) . "'";
			// 重置ec_salt、salt
			$values[] = "salt = 0";
			$values[] = "ec_salt = 0";
			...
			$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET `passwd_weixin` =  '$pwd' WHERE user_name = '$user_name'");
		}
		...
		if((! empty($cfg['email'])) && $this->field_email != 'NULL')
		{
			...
			$values1[] = $this->field_email . "='" . $cfg['email'] . "'";
		...
		if((! empty($cfg['mobile_phone'])) && $this->field_mobile_phone != 'NULL')
		{
			...
			$values1[] = $this->field_mobile_phone . "='" . $cfg[mobile_phone] . "'";
		...
		if($values)
		{
			...
			$sql="select count(user_id) from". $this->table("supplier_admin_user")." WHERE " . $this->field_name . " = '$cfg[post_username]' ";


			if($this->db->getOne($sql, true)> 0&&$values1){

   
	  
	  $sql = "UPDATE " . $this->table("supplier_admin_user") . " SET " . implode(', ', $values1) . " WHERE " . $this->field_name . "='" . $cfg['post_username'] . "' LIMIT 1";
