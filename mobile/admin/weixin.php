...
switch ($act){
	case "config":
		if($_POST){
		   admin_priv('weixin_config');
		    require_once(ROOT_PATH . 'includes/cls_image.php');
			$file_url = '';
			$file_plus = '';
		    if ((isset($_FILES['guide_qrcode']['error']) && $_FILES['guide_qrcode']['error'] == 0) || (!isset($_FILES['guide_qrcode']['error']) && isset($_FILES['guide_qrcode']['tmp_name']) && $_FILES['guide_qrcode']['tmp_name'] != 'none'))
		    {
		        if (!check_file_type($_FILES['guide_qrcode']['tmp_name'], $_FILES['guide_qrcode']['name'], $allow_file_types))
		        {
		            sys_msg($_LANG['invalid_file']);
		        }

		        $res = upload_article_file($_FILES['guide_qrcode']);
		        if ($res != false)
		        {
		            $file_url = $res;
		            $file_plus = " `guide_qrcode`	='$file_url', ";
		        }
		    }
			...
			$open_guide = getstr($_POST ['open_guide']);
			$appsecret = getstr($_POST ['appsecret']);
			$partnerId = getstr($_POST ['partnerId']);
			$partnerKey = getstr($_POST ['partnerKey']);
			$admin_id = getstr($_POST ['admin_id']);
			...
			// 今天优品多商户系统 技术支持：商派网络，QQ：309485552、120029121   20160717
			$old = $db->getRow ( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_config') . " WHERE `id` = 1" );
			$token=(substr_replace($old['token'],"*******",3)==$_POST['token']?$old['token']:$_POST['token']);
			$appid=substr_replace($old['appid'],"*******",4,7)==$_POST['appid']?$old['appid']:$_POST['appid'];
			$appsecret=substr_replace($old['appsecret'],"*******",4,7) ==$_POST['appsecret']?$old['appsecret']:$_POST['appsecret'];
			$partnerId=substr_replace($old['partnerId'],"*****",3,4) ==$_POST['partnerId']?$old['partnerId']:$_POST['partnerId'];
			$partnerKey=substr_replace($old['partnerKey'],"*******",8,16) ==$_POST['partnerKey']?$old['partnerKey']:$_POST['partnerKey'];
			// 今天优品多商户系统 技术支持：商派网络，QQ：309485552、120029121   20160717
			...
			$weixin_url ='http://'.$_SERVER['HTTP_HOST'].'/mobile/weixin/index.php';
			$wx_url = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/';
            $smarty->assign('weixin_url',    $weixin_url);
			$smarty->assign ( 'token', substr_replace($ret ['token'],"*******",3) );
			$smarty->assign ( 'appid', substr_replace($ret ['appid'],"*******",4,7) );
			$smarty->assign ( 'title', $ret ['title'] );
			$smarty->assign ( 'appsecret', substr_replace($ret ['appsecret'],"*******",4,7) );
			$smarty->assign ( 'partnerId', substr_replace($ret ['partnerId'],"*****",3,4) );
			$smarty->assign ( 'partnerKey', substr_replace($ret ['partnerKey'],"*******",8,16) );
			...
			$smarty->assign ( 'wap_url', $ret['wap_url'] );
			$smarty->assign ( 'wx_url', $wx_url);
			$smarty->assign ( 'admin_id',$ret ['admin_id']);
			$smarty->assign ( 'open_guide',$ret ['open_guide']);
			$smarty->assign ( 'guide_qrcode', $ret ['guide_qrcode'] );
	...
	if($_POST) 
	{
		...
		$rand = $_POST['rand'];
		$open_email = $_POST['open_email'];
		$email = $_POST['email'];
		$userpwd = $_POST['userpwd'];
		$update_sql = "UPDATE `ecs_weixin_autoreg` SET `autoreg_name` = '$auto_reg',`userpwd` = '$userpwd',`email` = '$email',`open_email` = '$open_email',`autoreg_rand` = '$rand' WHERE `autoreg_id` =1;";
		$db->query($update_sql);
		$ret = $db->query($update_sql);
		$link[] = array('href' =>'weixin.php?act=autoreg', 'text' => "微信自动注册设置");
		if($ret)
		{
			sys_msg('设置成功',0,$link);
		}
	...
	else 
	{
		...
		$data = $db->getRow("SELECT * FROM `ecs_weixin_autoreg` WHERE `autoreg_id` = 1");
	}
	
	$smarty->assign('ur_here',      "微信自动注册设置");
	$smarty->assign('data',$data);
//	$smarty->assign('full_page', 1);
	$smarty->display('weixin/wx_autoreg.html');
	break;
	case "query":
	case "fans":
	...
	case "notice":
		if($_POST){
			...
			$is_thumbnail 	= intval($_POST ['is_thumbnail']);
			$sharemsg = getstr($_POST['sharemsg']);
			$ret = $db->query (
					...
					`pengyou_money_up`		='$_POST[pengyou_money_up]',
					`pengyou_point_up`		='$_POST[pengyou_point_up]',
					`pengyouquan_money_up`	='$_POST[pengyouquan_money_up]',
					`pengyouquan_point_up`	='$_POST[pengyouquan_point_up]',
					`buynotice`			='$buynotice',".$file_plus."
					`sendnotice`		='$sendnotice'
					WHERE `id`=1;" );
					$link [] = array ('href' => 'weixin.php?act=notice','text' => '分享设置');
			if ($ret) {
					sys_msg ( '设置成功', 0, $link );
			} else {
				sys_msg ( '设置失败，请重试', 0, $link );
			...
			$smarty->assign ( 'pengyou_money_up', $ret ['pengyou_money_up'] );
			$smarty->assign ( 'pengyou_point_up', $ret ['pengyou_point_up'] );
			$smarty->assign ( 'pengyouquan_money_up', $ret ['pengyouquan_money_up'] );
			$smarty->assign ( 'pengyouquan_point_up', $ret ['pengyouquan_point_up'] );
		...
		if($_POST){
			...
			$buynotice = intval($_POST ['buynotice']);
			$buysuppliertice = intval($_POST ['buysuppliertice']);
			$buyuptice = intval($_POST ['buyuptice']);
			$paytice = intval($_POST ['paytice']);
			$paysuppliertice = intval($_POST ['paysuppliertice']);
			$payuptice = intval($_POST ['payuptice']);
			$jointice = intval($_POST ['jointice']);
			$sendnotice = intval($_POST ['sendnotice']);
			$lucky_buy_lucky_user_notice = intval($_POST ['lucky_buy_lucky_user_notice']);

			$buymsg = getstr($_POST['buymsg']);
			$buysuppliermsg = getstr($_POST['buysuppliermsg']);
			$buyupmsg = getstr($_POST['buyupmsg']);
			$paymsg = getstr($_POST['paymsg']);
			$paysuppliermsg = getstr($_POST['paysuppliermsg']);
			$payupmsg = getstr($_POST['payupmsg']);
			$joinmsg = getstr($_POST['joinmsg']);
			$sendmsg = getstr($_POST['sendmsg']);
			$lucky_buy_lucky_user_msg = getstr($_POST['lucky_buy_lucky_user_msg']);
			
            $extpintuan_open=getstr($_POST['extpintuan_open']);
            $extpintuan_join=getstr($_POST['extpintuan_join']);
            $extpintuan_success=getstr($_POST['extpintuan_success']);
            $extpintuan_fail=getstr($_POST['extpintuan_fail']);
            $extpintuan_result=getstr($_POST['extpintuan_result']);

		    $db->query ("UPDATE " . $GLOBALS['ecs']->table('weixin_new_remind') . " SET `templet_id`='$extpintuan_open' WHERE `remind_name` = 'extpintuan_open' " );
		    $db->query ("UPDATE " . $GLOBALS['ecs']->table('weixin_new_remind') . " SET `templet_id`='$extpintuan_join' WHERE `remind_name` = 'extpintuan_join' " );
		    $db->query ("UPDATE " . $GLOBALS['ecs']->table('weixin_new_remind') . " SET `templet_id`='$extpintuan_success' WHERE `remind_name` = 'extpintuan_success' " );
		    $db->query ("UPDATE " . $GLOBALS['ecs']->table('weixin_new_remind') . " SET `templet_id`='$extpintuan_fail' WHERE `remind_name` = 'extpintuan_fail' " );
		    $db->query ("UPDATE " . $GLOBALS['ecs']->table('weixin_new_remind') . " SET `templet_id`='$extpintuan_result' WHERE `remind_name` = 'extpintuan_result' " );
			
			$ret = $db->query (

					"UPDATE " . $GLOBALS['ecs']->table('weixin_remind') . " SET
					`buynotice`='$buynotice',
					`buymsg`='$buymsg',
					
					`buysuppliertice`='$buysuppliertice',
					`buysuppliermsg`='$buysuppliermsg',
					...
					`paysuppliertice`='$paysuppliertice',
					`paysuppliermsg`='$paysuppliermsg',
					...
					`lucky_buy_lucky_user_notice`='$lucky_buy_lucky_user_notice',
					`lucky_buy_lucky_user_msg`='$lucky_buy_lucky_user_msg'

					WHERE `id`=1;" );

					$link [] = array ('href' => 'weixin.php?act=remind','text' => '模板消息提醒设置');
			...
			$smarty->assign('ur_here',      "模板消息提醒设置");

			$ret = $db->getRow ( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_remind') . " WHERE `id` = 1" );

			$smarty->assign ( 'buynotice', $ret ['buynotice'] );
			$smarty->assign ( 'buymsg', $ret ['buymsg'] );
			
			$smarty->assign ( 'buysuppliertice', $ret ['buysuppliertice'] );
			$smarty->assign ( 'buysuppliermsg', $ret ['buysuppliermsg'] );
			
			$smarty->assign ( 'buyuptice', $ret ['buyuptice'] );
			$smarty->assign ( 'buyupmsg', $ret ['buyupmsg'] );
			
			$smarty->assign ( 'paytice', $ret ['paytice'] );
			$smarty->assign ( 'paymsg', $ret ['paymsg'] );
			
			$smarty->assign ( 'paysuppliertice', $ret ['paysuppliertice'] );
			$smarty->assign ( 'paysuppliermsg', $ret ['paysuppliermsg'] );
			
			$smarty->assign ( 'payuptice', $ret ['payuptice'] );
			$smarty->assign ( 'payupmsg', $ret ['payupmsg'] );
			
			$smarty->assign ( 'jointice', $ret ['jointice'] );
			$smarty->assign ( 'joinmsg', $ret ['joinmsg'] );

			$smarty->assign ( 'sendnotice', $ret ['sendnotice'] );
			$smarty->assign ( 'sendmsg', $ret ['sendmsg'] );

			$smarty->assign ( 'lucky_buy_lucky_user_notice', $ret ['lucky_buy_lucky_user_notice'] );
            $smarty->assign ( 'lucky_buy_lucky_user_msg', $ret ['lucky_buy_lucky_user_msg'] );
			
			$extpintuan_open = $db->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['ecs']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_open' " );
			$extpintuan_join = $db->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['ecs']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_join' " );
			$extpintuan_success = $db->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['ecs']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_success' " );
			$extpintuan_fail = $db->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['ecs']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_fail' " );
			$extpintuan_result = $db->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['ecs']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_result' " );
			
            $smarty->assign ( 'extpintuan_open', $extpintuan_open );
            $smarty->assign ( 'extpintuan_join', $extpintuan_join );
            $smarty->assign ( 'extpintuan_success', $extpintuan_success );
            $smarty->assign ( 'extpintuan_fail', $extpintuan_fail );
            $smarty->assign ( 'extpintuan_result', $extpintuan_result );
			
			$smarty->display ( 'weixin/wx_remind.html' );
