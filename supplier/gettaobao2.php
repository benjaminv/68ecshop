        ...
        $comment_list = getEvalution($userNumId,$id,$cnum);
			...
			if($comment_list){
				...
				foreach($comment_list as $k=>$c){
					$t = $t-832*$k;
					$db->query("insert into {$ecs->table('comment')} (id_value,content,comment_rank,add_time,user_name,status,supplier_id) 
					value ($gid,'{$c['feedback']}',5,'{$t}','{$c['nick']}',1,'{$_SESSION['supplier_id']}')");
